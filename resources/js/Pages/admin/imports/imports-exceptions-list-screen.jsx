...
export default function ImportsExceptionsListScreen({ batch, exceptions, filters }) {
    function resolve(exception, resolution) {
        const note = window.prompt('Optional note for this resolution:') ?? '';
        router.patch(route('portal.imports.exceptions.resolve', [batch.id, exception.id]), {
            resolution,
            note: note || null,
        });
    }
