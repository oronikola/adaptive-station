...
export default function ManageSchoolModal({
    ...
    const [revealedAdminPasswords, setRevealedAdminPasswords] = useState<Set<string>>(
        () => new Set(),
    );
    const [copiedAdminPasswordId, setCopiedAdminPasswordId] = useState<string | null>(null);
    ...
    useEffect(() => {
        if (!show || !tenant) {
            ...
            setOverviewErrors({});
            setAdminErrors({});
            setRevealedAdminPasswords(new Set());
            setCopiedAdminPasswordId(null);
            return;
        }
        ...
        try {
            ...
            if (res.data.admin) {
                setAdmins((prev) => [
                    ...
                    {
                        ...res.data.admin,
                        password_plaintext: res.data.temporaryPassword ?? null,
                    },
                ]);
            }
    ...
    function toggleAdminPassword(adminId: string) {
        setRevealedAdminPasswords((current) => {
    ...
    async function handleCopyAdminPassword(admin: Admin) {
        if (!admin.password_plaintext) {
            return;
        }

        await handleCopySecret(admin.password_plaintext);
        setCopiedAdminPasswordId(admin.id);
        setTimeout(() => setCopiedAdminPasswordId(null), 2000);
    }
                        ...
                        {newAdminPassword && (
                            <div className="overflow-hidden rounded-2xl border border-amber-300/80 bg-[#071c44] p-4 text-white shadow-lg">
                                ...
                                <p className="mt-2 text-[11px] text-slate-300">
                                    Copy this password for the school administrator. It remains available from the Password column below.
                                </p>
                            </div>
                        )}
                                    ...
                                    {isLoadingDetails ? (
                                        ...
                                        admins.map((adm) => (
                                            <tr key={adm.id}>
                                                ...
                                                <td>
                                                    {adm.password_plaintext ? (
                                                        <div className="flex items-center gap-1.5">
                                                            <code className="min-w-24 rounded-lg bg-slate-100 px-2 py-1 font-mono text-[11px] text-slate-700 dark:bg-slate-800 dark:text-slate-200">
                                                                {revealedAdminPasswords.has(adm.id)
                                                                    ? adm.password_plaintext
                                                                    : '••••••••••••'}
                                                            </code>
                                                            <button
                                                                type="button"
                                                                onClick={() => toggleAdminPassword(adm.id)}
                                                                className="inline-flex h-7 w-7 items-center justify-center rounded-lg text-slate-500 transition hover:bg-slate-100 hover:text-blue-600 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-blue-400"
                                                                aria-label={revealedAdminPasswords.has(adm.id) ? `Hide password for ${adm.email}` : `Show password for ${adm.email}`}
                                                                title={revealedAdminPasswords.has(adm.id) ? 'Hide password' : 'Show password'}
                                                            >
                                                                {revealedAdminPasswords.has(adm.id) ? (
                                                                    <EyeOffIcon size={15} />
                                                                ) : (
                                                            ...
                                                            <button
                                                                type="button"
                                                                onClick={() => handleCopyAdminPassword(adm)}
                                                                className="inline-flex h-7 items-center gap-1 rounded-lg px-2 text-[10px] font-semibold text-slate-500 transition hover:bg-slate-100 hover:text-blue-600 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-blue-400"
                                                                aria-label={`Copy password for ${adm.email}`}
                                                                title="Copy password"
                                                            >
                                                                <CopyIcon size={13} />
                                                                {copiedAdminPasswordId === adm.id ? 'Copied' : 'Copy'}
                                                            </button>
                                                        </div>
                                                    ) : (
