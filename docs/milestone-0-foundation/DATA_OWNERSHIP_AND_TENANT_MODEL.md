...
## Update — 2026-09-14: essentiel-sourced clients get a live, opt-in integration

Two of the constraints above were already superseded by later shipped work, and a new one is added by this update — recorded here rather than silently rewritten, so the decision trail stays intact:

- **Per-tenant database**: the "single shared MySQL 8 database" model in the Tenant Model section above was superseded before this update — Adaptive Station now provisions a dedicated physical database per tenant (see `Tenant::provision()`/`TenantDatabase`). The central `mysql` connection remains for cross-tenant tables (users, tenants, `sms_outbox`, etc.).
- **Legacy integration direction**: "IP-005 must build the one-way legacy export (not bidirectional)" was already superseded by the shipped `legacy_mysql` driver, which supports `import_only`/`export_only`/`bidirectional` profiles, including a real-time per-tap push (`PushTapEventToLegacyJob`) alongside historical import (`RunLegacyImportJob`).
- **New decision — essentiel-sourced clients**: a school onboarded specifically as an essentiel-affiliated client (as opposed to a school migrating *off* a legacy system) may use a new `essentiel_api` integration driver (`EssentielApiConnector`, `PushTapEventToEssentielJob`). On every tap, this driver calls essentiel's `POST /api/v1/tapping/record` to resolve the RFID and record the tap on essentiel's side, and Adaptive Station sends the guardian SMS itself using the phone number that response returns. This is **opt-in per tenant** (an active `essentiel_api` `IntegrationProfile`) — every other tenant is entirely unaffected and keeps the original local-only notification flow (`DispatchParentTapNotification` + the `legacy_mysql` driver, if configured).
- **The `tapbunker` rule still stands, unchanged**: this integration never reads or writes essentiel's `tapbunker` SMS log table. Adaptive Station's own SMS gateway remains the single delivery owner for every tenant, essentiel-sourced or not — essentiel is consulted only for identity/guardian *lookup* and its own `taphistory` record-keeping, never for sending the message.
